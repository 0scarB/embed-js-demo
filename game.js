...
function collideBallWithRect(rectX, rectY, rectWidth, rectHeight) {
    const rMinX = rectX;
...
function update(frameTimestamp) {
    ...
    for (let i = 0; i < bricksCount * 2; i += 2) {
        if (collideBallWithRect(bricksPos[i], bricksPos[i + 1], BRICK_WIDTH, BRICK_HEIGHT)) {
...
function render() {
    ...
    // Draw paddle
    canvasCtx.fillRect(paddleX * gameToScreenScaleFactor, (GAME_HEIGHT - PADDLE_HEIGHT) * gameToScreenScaleFactor, PADDLE_WIDTH * gameToScreenScaleFactor, PADDLE_HEIGHT * gameToScreenScaleFactor);
    // Draw bricks
    for (let i = 0; i < bricksCount * 2; i += 2) {
        const x = bricksPos[i];
        const y = bricksPos[i + 1];
        canvasCtx.fillRect(x * gameToScreenScaleFactor, y * gameToScreenScaleFactor, BRICK_WIDTH * gameToScreenScaleFactor, BRICK_HEIGHT * gameToScreenScaleFactor);
    }
    // Draw ball
    canvasCtx.beginPath();
    canvasCtx.arc(ballX * gameToScreenScaleFactor, ballY * gameToScreenScaleFactor, BALL_RADIUS * gameToScreenScaleFactor, 0, 2 * Math.PI);
    canvasCtx.fill();
}
function resize() {
...
function gameLoop(frameTimestamp) {
